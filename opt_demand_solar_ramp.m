function [beta_demand, alpha_demand, r_solar_M_H, pred_demand] = ...
    opt_demand_solar_ramp(quant, Demand, Date, Holidays, Solar,lambda)
...
%     ob5 = sum( ob5_pre .* ob5_pre);
    ...
    ob4_pre = r_solar_matrix-2*circshift(r_solar_matrix,1)+circshift(r_solar_matrix,2);
...
%     ob7 = sum(ob7_pre .* ob7_pre);
    ...
    minimize( ob1 + lambda1 * ob2 + lambda2 * ob3 + lambda3 * ob4 + lambda4 * ob5 + lambda5*ob6);
