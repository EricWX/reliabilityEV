...
load_train = [load2016_r, load2017_r]'/max(load2018_r) * 1000 /.93;
...
%% solar ramp
lambda = [10^(5), 10^(2)]; % HyperParameter
[beta_solar_ramp,alpha_solar_ramp, pred_solar_ramp] =opt_solar_ramp(quant, diff(solar13_17), date13_17(2:end),lambda);

%% demand ramp
% lambda = [10^(3),10^(0),10^(7),10^(1),0];
% 
% [beta_demand_ramp, alpha_demand_ramp, r_demand_solar_M_H_ramp, pred_demand_ramp] = ...
%     opt_demand_solar_ramp(quant, diff(load2018), date18(2:end),...
%     Holidays_18(2:end),  diff(solar2018),lambda);%lambda 7 parameters

lambda = [10^(1),10^(0),10^(-2), 0,0];
lambda =[10^(1),10^(0),10^(3), 10^(3), 10^(1)]
[beta_demand_ramp, alpha_demand_ramp, r_demand_solar_M_H_ramp, pred_demand_ramp] = ...
    opt_demand_solar_ramp(quant, diff(load_train), date16_17(2:end),...
    Holidays_16_17(2:end),  diff(solar16_17), lambda);
...
[paramEsts_left_D_ramp,paramEsts_right_D_ramp,Error_left_pos_D_ramp,Error_right_pos_D_ramp] = ...
    demand_tail_est(tail_n,quant,pred_demand_ramp, diff(load_train));

[paramEsts_left_S_ramp,paramEsts_right_S_ramp,Error_left_pos_S_ramp,Error_right_pos_S_ramp] = ...
    demand_tail_est(tail_n,quant,pred_solar_ramp, diff(solar13_17));
...
[beta_demand, alpha_demand, r_demand_solar_M_H, pred_demand] = ...
    opt_demand_solar_power(quant, load_test, date18,...
...
[paramEsts_left,paramEsts_right,Error_left_pos,Error_right_pos] = ...
    demand_tail_est(tail_n,quant,pred_demand, load_test);
...
ramp_capacity = (10+76.93+4.33+3.4+2.9+3.15) * 60 + 1000;
...
% Solar QR to PDF
    ...
    x_G_L_W_S = x_G_L_W_S + min_demand + min_solar + power_capacity;
...
listOfProb_up = zeros(length(date18),1);
...
for i = 1 : length(date18)
    ...
    listOfProb_up(i) = p_ramp_down(a);
    listOfProb_down(i) = p_ramp_up(a);
    
    listOfProb_power(i) = p_power(a);
end
...
% Solar QR to PDF
    ...
    if abs(solar_scale(end) - solar_scale(1)) <= 24
        ...
        [q_solar_comb,solar_sample] = tailSample_new(quant, solar_tilde,...
            paramEsts_right_S_ramp, paramEsts_left_S_ramp);
        solar_sample = solar_sample * solar_factor;
        if solar_factor < 0
           solar_sample = fliplr(solar_sample);
            q_solar_comb =1- fliplr(q_solar_comb);
        end
